...
public final class Tuple2<T1, T2> implements Tuple {
	...
	public Tuple2<T1, T2> filter(Predicate2<? super T1, ? super T2> filter) {
		...
		if (filter.satisfy(this.value1, this.value2)) {
			return this;
		} else {
			return empty();
		}
	...
	public <U> Tuple1<U> map(Function2<? super T1, ? super T2, ? extends U> mapper) {
		...
		if (Objects.isNull(this.value1) && Objects.isNull(this.value2)) {
			return Tuple1.empty();
		} else {
			return new Tuple1<>(mapper.apply(this.value1, this.value2));
		}
	}
